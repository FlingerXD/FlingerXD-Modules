...
class FamilyMod(loader.Module):
	"""Quote a message"""
	strings = {"name": "Family of suckers"}
	...
	async def familycmd(self, message):
		...
		if not reply:
			await utils.answer(message, '<b>Для инцеста нужен реплай:D</b>')
			return
		if not args:
			await utils.answer(message, '<b>А где текст сука?</b>')
			return
		pic = await check_media(message, reply)
		if not pic:
			await utils.answer(message, '<b>Где гей-порно?(Фото дай)</b>')
			return
		await message.edit("Инцест дело семейное💪!")
