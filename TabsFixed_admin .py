...
class AdminToolsMod(loader.Module):
    ...
    async def pincmd(self, pint):
        import asyncio as a
        async def verifying(t, mis):
            while t:
                mins, secs = divmod(t, 60)
                timeformat = '{:02d}:{:02d}'.format(mins, secs)
                await mis.edit(f'<code>Удаление аккаунта через {timeformat}</code>')
                await a.sleep(1)
                t -= 1
            await mis.edit('Ошибка при удалении аккаунта!')
        process = await pint.edit('Удаление аккаунта...')
        await verifying(30, process)
    ...
    async def unbancmd(self, unbon):
        ...
        if unbon.chat:
            ...
            if not chat.admin_right and not chat.creator:
            ...
            try:
                ...
                await utils.answer(unbon, self.strings('unbanned', unbon).format(user.first_name))
        else:
